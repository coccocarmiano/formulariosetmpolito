...
{\bf Errore Qualunque (Derivate Parziali):} $\displaystyle y = f(x_1, x_2, ...) \to \delta y  = \sum_i \Bigg |\frac{\partial f(x)}{\partial x_i}\Bigg |_{x=x_{mis}} \delta x_i $
...
{\bf Full-Scale Range:} $ FSR = V_{max} - V_{min} $ \ \ \ {\bf Quantizzazione su N bit:} $ N = log_2(M) $
...
{\bf Ponte di Wheatstone:} ? \ \ \ {\bf Potenza} $\displaystyle P = \frac{v_{eff}^2}{R} $, per segnali sinusoidali $\displaystyle v_{eff} = \frac{A}{\sqrt{2}}$
...
{\bf Diodo Reale:} $\displaystyle i_D = I_s(e^{\frac{v_D}{\eta V_T}}- 1)$, curva quasi verticale per $ v_D > V_\gamma \simeq 0.7 \ V $
...
{\bf Corrente di Gate:} $i_G = 0$ {\bf sempre}.
...
{\bf Common Source:} $ A_v = -g_m(R || r_o), \ R_{in} \to \infty \ R_{out} = R || r_o$

{\bf Common Drain:} $ A_v = \displaystyle \frac{g_mR'}{1+g_mR'} < \ R_{in} \to \infty \ R_{out} \simeq \frac{1}{g_m}$	

{\bf Common Gate:} $ A_v \simeq g_m(R || r_o) \ R_{in} \simeq \frac{1}{g_m} \ R_{out} = R || r_o$ 

{\bf Sia PMOS che NMOS:} il Gate è sempre la "sbarra", il Source sempre dove sta la corrente, il Drain è l'altro per esclusione
...
{\bf Amplificatore di Transconduttanza:} $\displaystyle G_m = \frac{1}{R}, R_{in} \to \infty, R_{out} \to \infty$

{\bf Amplificatore di Transresistenza: } $\displaystyle R_m = R, R_{in} \to 0, R_{out} \to 0$
...
{\bf Sommatore Generalizzato:} $\displaystyle v_{out} = \frac{\sum_{i=0}^M G_{i^-} + G_f}{\sum_{i=0}^N G_{i^+}} \sum_{i=1}^N \frac{G_{i^+}}{G_f}v_{i^+}-\sum_{i=1}^M\frac{G_{i^-}}{G_f}v_{i^-}$ (Millman...)
